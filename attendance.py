...
Path(trainimage_path).mkdir(parents=True, exist_ok=True)
Path(attendance_path).mkdir(parents=True, exist_ok=True)
(BASE_DIR / 'StudentDetails').mkdir(parents=True, exist_ok=True)
(BASE_DIR / 'TrainingImageLabel').mkdir(parents=True, exist_ok=True)
if not Path(studentdetail_path).exists():
    Path(studentdetail_path).write_text("Enrollment,Name\n")
...
def err_screen():
    ...
    sc1.iconbitmap(str(BASE_DIR / 'AMS.ico'))
    sc1.title("Warning!!")
...
logo = Image.open(BASE_DIR / 'UI_Image' / '0001.png')
...
ri = Image.open(BASE_DIR / 'UI_Image' / 'register.png')
r = ImageTk.PhotoImage(ri)
label1 = Label(window, image=r)
label1.image = r
label1.place(x=100, y=270)

ai = Image.open(BASE_DIR / 'UI_Image' / 'attendance.png')
a = ImageTk.PhotoImage(ai)
label2 = Label(window, image=a)
label2.image = a
label2.place(x=980, y=270)

vi = Image.open(BASE_DIR / 'UI_Image' / 'verifyy.png')
